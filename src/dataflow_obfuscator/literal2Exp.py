...
class literal2Exp:
	...
	def getNodeInfor(self, _node):
		try:
			# 新版AST: value字段在顶级, typeString在typeDescriptions中
			# 旧版AST: 在attributes.type中
			value = _node.get("value")
			if not value:
				type_str = _node.get("attributes", {}).get("type", "")
				value = type_str.split()[1] if len(type_str.split()) > 1 else None
			if not value:
				return 0, 0, 0
	...
	def doGenerate(self, _prob):
		#1. find each literal
		literalList = self.findASTNode("nodeType", "Literal")
