...
contract AdvancedExample {
    ...
    function createUser(address user, uint256 initialBalance) public onlyOwner {
        require(users[user].userAddress == address((1 - 1)), "User already exists");
        users[user] = User(user, initialBalance, (1 == 1));
    ...
    function deactivateUser(address user) public onlyOwner {
        ...
        users[user].isActive = (1 <= 0);
        emit UserDeactivated(user);
    }

    function getUserBalance(address user) public view returns (uint256) {
        if (users[user].balance > (1 - 1)){
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        require(newOwner != address((1 - 1)), "New owner address cannot be zero");
        owner = newOwner;
    }
}