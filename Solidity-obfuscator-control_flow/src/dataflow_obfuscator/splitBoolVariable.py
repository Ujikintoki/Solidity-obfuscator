...
class splitBoolVariable:
	...
	def findBoolList(self, _prob):
		nodeList = self.findASTNode("nodeType", "Literal")
		#nodeList.extend(self.findASTNode("name", "VariableDeclarationStatement"))
		resultList = list()
		for node in nodeList:
			try:
				# 新版AST: typeDescriptions.typeString 和 value 在顶级
				# 旧版AST: attributes.type 和 attributes.value
				type_str = node.get("typeDescriptions", {}).get("typeString", "")
				if not type_str:
					type_str = node.get("attributes", {}).get("type", "")
				value = node.get("value")
				if not value:
					value = node.get("attributes", {}).get("value")
				if type_str == "bool" and random() < _prob:
	...
	def findStatement(self):
		resultList = list()
		resultList.extend(self.findASTNode("nodeType", "VariableDeclaration"))
		resultList.extend(self.findASTNode("nodeType", "VariableDeclarationStatement"))
		resultList.extend(self.findASTNode("nodeType", "Assignment"))
		return resultList
	...
	def findAssign(self, _boolList, _dict):
		for _bool in _boolList:
			try:
				# 新版AST: operator 在顶级, children可能不存在
				ope = _dict.get("operator")
				if not ope:
					ope = _dict.get("attributes", {}).get("operator")
				# 访问左侧表达式
				left_expr = _dict.get("leftHandSide") or _dict.get("children", [None])[0]
				if left_expr:
					_id = left_expr.get("referencedDeclaration") or left_expr.get("attributes", {}).get("referencedDeclaration")
					name = left_expr.get("name") or left_expr.get("attributes", {}).get("value")
					#print(ope, _id, name)
	...
	def findVarDeclare(self, _boolList, _dict):
		for _bool in _boolList:
			try:
				...
				name = _dict.get("name")
				if not name:
					name = _dict.get("attributes", {}).get("name")
	...
	def findVarDeclareState(self, _boolList, _dict):
		for _bool in _boolList:
			try:
				# 新版AST: assignments 可能在顶级
				assignments = _dict.get("assignments", [])
				if not assignments:
					assignments = _dict.get("attributes", {}).get("assignments", [])
				if _bool[1] in assignments:
